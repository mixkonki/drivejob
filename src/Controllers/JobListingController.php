...
use Drivejob\Models\ProfileModel;
...
class JobListingController
{
    ...
    public function show($id)
    {
        try {
            ...
            Logger::error('Error in JobListingController::show: ' . $e->getMessage());
    ...
    /**
     * Εμφανίζει τη φόρμα δημιουργίας νέας αγγελίας
     */
    public function create()
    {
        // Έλεγχος αν ο χρήστης είναι συνδεδεμένος
        if (!isset($_SESSION['user_id']) || !isset($_SESSION['role'])) {
            $_SESSION['error_message'] = 'Πρέπει να συνδεθείτε για να δημιουργήσετε αγγελία';
            header('Location: ' . BASE_URL . 'login.php');
            exit();
        }

        // Έλεγχος της μεθόδου αιτήματος
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            // Επεξεργασία της φόρμας δημιουργίας αγγελίας
            return $this->store();
        }

        // Ανάλογα με τον ρόλο του χρήστη, φορτώνουμε διαφορετικό template
        if ($_SESSION['role'] === 'driver') {
            // Φόρτωση του template για οδηγούς
            include ROOT_DIR . '/src/Views/job-listings/create-driver.php';
        } else if ($_SESSION['role'] === 'company') {
            // Φόρτωση του template για εταιρείες
            include ROOT_DIR . '/src/Views/job-listings/create-company.php';
        } else {
            // Ο χρήστης δεν έχει δικαίωμα δημιουργίας αγγελίας
            $_SESSION['error_message'] = 'Δεν έχετε δικαίωμα δημιουργίας αγγελίας';
            header('Location: ' . BASE_URL . 'job-listings');
            exit();
        }
    }
    ...
    public function store()
    {
        ...
        // Ανάλογα με τον ρόλο του χρήστη, προσθέτουμε επιπλέον πεδία
        if ($_SESSION['role'] === 'driver') {
        ...
        $vehicleTypes = $_POST['vehicle_types'] ?? [];
        ...
        if ($jobListingId) {
            ...
            if (!empty($vehicleTypes)) {
                $this->jobListingModel->updateVehicleTypes($jobListingId, $vehicleTypes);
            }

            // Προσθήκη των tags
            if (isset($_POST['tags']) && is_array($_POST['tags'])) {
                $jobTagModel = new \Drivejob\Models\JobTagModel($this->pdo);
                $jobTagModel->updateListingTags($jobListingId, $_POST['tags']);
            }

            $_SESSION['success_message'] = 'Η αγγελία δημιουργήθηκε με επιτυχία';
            header('Location: ' . BASE_URL . 'job-listings/' . $jobListingId);
            exit();
        } else {
            $_SESSION['error_message'] = 'Υπήρξε ένα σφάλμα κατά τη δημιουργία της αγγελίας';
            header('Location: ' . BASE_URL . 'job-listings/create');
            exit();
        }
    }
