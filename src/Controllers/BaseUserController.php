...
class BaseUserController extends BaseController
{
    ...
    protected function collectRegistrationData($userType)
    {
        ...
        if ($userType === 'company') {
            ...
            $data['legal_status'] = (($_POST['legal_status'] ?? '') === 'yes') ? 'yes' : 'no';
        }
    ...
    protected function validateRegistrationData($data, $userType)
    {
        ...
        /*
         * ΟΙ ΚΑΝΟΝΕΣ ΣΥΝΘΗΜΑΤΙΚΟΥ ΠΡΕΠΕΙ ΝΑ ΤΑΙΡΙΑΖΟΥΝ ΜΕ ΟΣΑ ΥΠΟΣΧΕΤΑΙ Η ΦΟΡΜΑ.
         *
         * Η φόρμα γράφει τέσσερις απαιτήσεις (8-16 χαρακτήρες, κεφαλαίο,
         * αριθμός, ειδικός χαρακτήρας). Ο server έλεγχε ΜΟΝΟ το «τουλάχιστον
         * 8 χαρακτήρες». Δηλαδή ένα συνθηματικό «password» περνούσε, ενώ ο
         * χρήστης διάβαζε ότι δεν επιτρέπεται — και το αντίστροφο: ένα
         * συνθηματικό 20 χαρακτήρων απορριπτόταν αργότερα χωρίς εξήγηση.
         */
        $validator->required('email', 'Το email είναι υποχρεωτικό.')
            ...
            ->matches('password', 'confirm_password', 'Τα δύο συνθηματικά δεν ταιριάζουν.')
        ...
        /*
         * ΟΛΟΙ ΟΙ ΚΑΝΟΝΕΣ ΣΥΝΘΗΜΑΤΙΚΟΥ ΣΕ ΕΝΑ ΜΗΝΥΜΑ.
         *
         * Ο Validator κρατά ΕΝΑ σφάλμα ανά πεδίο — κάθε νέος κανόνας
         * αντικαθιστά τον προηγούμενο. Με πέντε ξεχωριστούς ελέγχους ο
         * χρήστης έβλεπε μόνο τον τελευταίο που απέτυχε: διόρθωνε τον
         * ειδικό χαρακτήρα, υπέβαλλε, και τότε μάθαινε ότι λείπει και
         * κεφαλαίο. Πέντε γύροι για ένα συνθηματικό.
         *
         * Εδώ ελέγχονται όλα μαζί και λέγεται με μία πρόταση τι λείπει.
         */
        ...
        if ($password !== '' && !isset($errors['password'])) {
            ...
            if ($length < 8 || $length > 16) {
                $missing[] = '8 έως 16 χαρακτήρες';
            }
            if (!preg_match('/[A-ZΑ-ΩΆΈΉΊΌΎΏ]/u', $password)) {
                $missing[] = 'ένα κεφαλαίο γράμμα';
            }
            if (!preg_match('/[0-9]/', $password)) {
                $missing[] = 'έναν αριθμό';
            }
            if (!preg_match('/[^\p{L}\p{N}]/u', $password)) {
                $missing[] = 'έναν ειδικό χαρακτήρα';
            }

            if ($missing !== []) {
                $last = array_pop($missing);
                $list = $missing === [] ? $last : implode(', ', $missing) . ' και ' . $last;
                $errors['password'] = 'Το συνθηματικό πρέπει να έχει ' . $list . '.';
            }
        }

        /*
         * ΤΟ EMAIL ΠΟΥ ΥΠΑΡΧΕΙ ΗΔΗ — Η ΠΙΟ ΣΥΧΝΗ ΑΠΟΤΥΧΙΑ ΕΓΓΡΑΦΗΣ.
         *
         * Δεν ελεγχόταν ΚΑΘΟΛΟΥ εδώ. Η εγγραφή προχωρούσε μέχρι το
         * registerDriver(), που επέστρεφε false, και ο χρήστης έβλεπε:
         *
         *     «Υπήρξε ένα σφάλμα κατά την εγγραφή σας. Δοκιμάστε ξανά.»
         *
         * Καμία ένδειξη ότι απλώς έχει ήδη λογαριασμό. Δοκίμαζε ξανά, και
         * ξανά, με το ίδιο αποτέλεσμα — κανένα «δοκίμασε ξανά» δεν επρόκειτο
         * ποτέ να δουλέψει.
         *
         * Το λέμε ρητά και δίνουμε τη διέξοδο. Για φόρμα ΕΓΓΡΑΦΗΣ αυτό είναι
         * σωστό: ο χρήστης πρέπει να μάθει ότι το email είναι πιασμένο,
         * αλλιώς κολλάει. (Σε φόρμα σύνδεσης θα ήταν λάθος — εκεί μια τέτοια
         * απάντηση αποκαλύπτει σε τρίτους ποιοι έχουν λογαριασμό.)
         */
        ...
        if ($email !== '' && !isset($errors['email'])) {
            try {
                if ($this->authModel->emailExists($email)) {
                    $errors['email'] = 'Υπάρχει ήδη λογαριασμός με αυτό το email. '
                        . 'Συνδέσου, ή ζήτησε νέο συνθηματικό αν το ξέχασες.';
                }
            } catch (\Throwable $e) {
                // Αποτυχία του ελέγχου δεν πρέπει να μπλοκάρει την εγγραφή:
                // το διπλότυπο θα πιαστεί ούτως ή άλλως από τη βάση.
                Logger::warning('Αδυναμία ελέγχου διπλότυπου email', ['message' => $e->getMessage()]);
            }
        }
