...
class BaseJobApplicationController extends BaseController
{
    ...
    public function viewApplication($id)
    {
        ...
        try {
             ...
             * οριζόταν σε ΚΑΝΕΝΑ σημείο του κώδικα — αναζήτηση σε όλο το
             * src/ επέστρεψε μηδέν. Ο οδηγός έβλεπε «Σε αναμονή» επ' άπειρον,
             * χωρίς να μάθει ποτέ αν κάποιος άνοιξε καν την αίτησή του.
             *
             * Στην αγορά εργασίας των μεταφορών, όπου ο οδηγός στέλνει
             * αιτήσεις σε δέκα εταιρείες, η σιωπή είναι το χειρότερο σήμα.
             * Μια ένδειξη «είδαν την αίτησή σου» κρατά τον χρήστη στην
             * πλατφόρμα — και δεν κοστίζει τίποτα, γιατί η πληροφορία
             * υπάρχει ήδη τη στιγμή που η εταιρεία ανοίγει τη σελίδα.
             *
             * Σημειώνεται μόνο για την εταιρεία και μόνο από την αρχική
             * κατάσταση: η προεπιλογή ή η απόρριψη δεν γυρίζουν πίσω.
             */
            if ($userRole === 'company' && ($application['status'] ?? '') === 'pending') {
                try {
                    ...
                    // Η σήμανση είναι ευγένεια, όχι προϋπόθεση — αν αποτύχει,
                    // η σελίδα πρέπει να ανοίξει κανονικά.
