...
class UnifiedJobListingController extends BaseJobListingController
{
    ...
    protected function collectFormData()
    {
        ...
        /*
         * ══════════════════════════════════════════════════════════════
         *  ΣΥΝΤΕΤΑΓΜΕΝΕΣ ΑΠΟ ΤΗΝ ΤΟΠΟΘΕΣΙΑ (31/08/2026)
         * ══════════════════════════════════════════════════════════════
         *
         * Χωρίς αυτές το ταίριασμα ΑΓΝΟΕΙ την απόσταση: το σκορ της
         * απόστασης δεν είναι χαμηλό, εξαφανίζεται από τον υπολογισμό.
         * Στην παραγωγή είχαμε 26 από 29 αγγελίες χωρίς συντεταγμένες,
         * και οδηγοί Θεσσαλονίκης έβλεπαν αγγελίες Αθηνών με 51%.
         *
         * Ένα migration γέμισε τις υπάρχουσες· εδώ φροντίζουμε να μην
         * ξαναδημιουργηθεί το πρόβλημα με κάθε νέα καταχώρηση.
         *
         * Ό,τι δεν αναγνωρίζεται (χωριό εκτός καταλόγου) μένει NULL —
         * η αγγελία δημοσιεύεται κανονικά, απλώς δεν κερδίζει βαθμούς
         * απόστασης. Ποτέ δεν μπλοκάρουμε δημοσίευση για γεωγραφία.
         */
        $place = \Drivejob\Helpers\GreekPlaces::locate($data['location']);
        if ($place) {
            $data['latitude'] = $place[0];
            $data['longitude'] = $place[1];
        }
