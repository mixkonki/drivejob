...
// Έλεγχος διαδρομής προτύπων
        ...
        // Έλεγχος αν υπάρχει πρότυπο email
            ...
            $this->log("Έλεγχος ρυθμίσεων email", 'INFO', [
                'smtp_host' => $this->emailService->getHost(),
                'sender_email' => $this->emailService->getSenderEmail(),
                'debug_mode' => $this->emailService->isDebugModeEnabled()
            ]);
            $this->log("Έλεγχος ρυθμίσεων SMS", 'INFO', [
                'sms_api_url' => $this->smsService->getApiUrl(),
                'sms_sender' => $this->smsService->getSender(),
                'debug_mode' => $this->smsService->isDebugMode()
            ]);
...
// Έλεγχος αν έχει ήδη σταλεί ειδοποίηση
                        ...
                        $message = $this->getEmailTemplate('driving_license', [
...
// Έλεγχος για ΠΕΙ Κατηγορίας C
            $sentNotifications = array_merge($sentNotifications, $this->checkSpecificPeiType('C', 'pei_expiry_c', $currentDate));
// Έλεγχος για ΠΕΙ Κατηγορίας D
            $sentNotifications = array_merge($sentNotifications, $this->checkSpecificPeiType('D', 'pei_expiry_d', $currentDate));
            return $sentNotifications;
        } catch (PDOException $e) {
...
// Έλεγχος αν έχει ήδη σταλεί ειδοποίηση για τη συγκεκριμένη περίοδο
                        ...
                        $message = $this->getEmailTemplate('pei', [
...
// Έλεγχος αν έχει ήδη σταλεί ειδοποίηση για τη συγκεκριμένη περίοδο
                        ...
                        $message = $this->getEmailTemplate('adr_certificate', [
...
// Έλεγχος αν έχει ήδη σταλεί ειδοποίηση για τη συγκεκριμένη περίοδο
                        ...
                        $message = $this->getEmailTemplate('tachograph_card', [
...
// Προετοιμασία του email
                        ...
                        $message = $this->getEmailTemplate('operator_license', [
                                'first_name' => $license['first_name'],
...
// Έλεγχος αν έχει ήδη σταλεί ειδοποίηση για τη συγκεκριμένη περίοδο
                        ...
                        $message = $this->getEmailTemplate('special_license', [
                                'first_name' => $license['first_name'],
...
// Δημιουργία του πίνακα αν δεν υπάρχει
        ...
        // Έλεγχος για duplicate key error
            if (strpos($e->getMessage(), 'Duplicate entry') !== false || $e->getCode() == 23000) {
                ...
                return true;
// Θεωρούμε επιτυχία καθώς η εγγραφή υπάρχει ήδη
            }
...
// Αν δεν υπάρχει ούτε το γενικό, το δημιουργούμε
        ...
        return $this->generateLicenseExpiryEmailTemplate($data['first_name'], $this->getLicenseCategoryDesc($licenseCategory), $data['license_type'] ?? ($data['adr_type'] ?? ($data['pei_category'] ?? 'Γενική')), $data['expiry_date'], $data['days_before_expiry']);
    }
 ...
 * @param mixed $data Προαιρετικά δεδομένα για καταγραφή
 */
