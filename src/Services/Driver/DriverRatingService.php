...
class DriverRatingService
{
    ...
    public function calculateQualificationsScore($driverId)
    {
        try {
            ...
            if (!empty($adrCertificates)) {
                $adrCertificate = $adrCertificates[0]; // Χρησιμοποιούμε το πρώτο πιστοποιητικό
    ...
    public function calculateSafetyScore($driverId)
    {
        try {
            ...
            // 1. Λήψη συμβάντων του οδηγού
            $incidents = $this->incidentModel->getDriverIncidents($driverId);

            // Φιλτράρισμα για τα συμβάντα των τελευταίων 3 ετών
            $threeYearsAgo = date('Y-m-d', strtotime('-3 years'));
            $incidents = array_filter($incidents, function ($incident) use ($threeYearsAgo) {
                return isset($incident['incident_date']) && $incident['incident_date'] >= $threeYearsAgo;
            });
