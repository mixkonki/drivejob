...
require_once dirname(__DIR__, 3) . "/src/RBAC/Perms.php";
...
<body>
    ...
    <script>
        ...
        async function getCSRF() {
            const r = await fetch("../api/legacy/csrf-token?uid=<?= $uid ?>");
        ...
        async function search() {
            ...
            const r = await fetch("../api/admin/users-overview?uid=<?= $uid ?>&limit=100&q=" + encodeURIComponent(q));
            const j = await r.json();
        ...
        async function callLinker(action) {
            ...
            const r = await fetch("../api/admin/link-identity?uid=<?= $uid ?>", {
                method: "POST",
                headers: {
