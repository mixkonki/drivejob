...
class CSRF
{
    ...
    /**
     * Το τρέχον token — παράγεται μόνο αν δεν υπάρχει ή έχει λήξει.
     *
     * ══════════════════════════════════════════════════════════════════════
     *  ΓΙΑΤΙ ΧΡΕΙΑΖΕΤΑΙ ΞΕΧΩΡΙΣΤΗ ΜΕΘΟΔΟΣ
     * ══════════════════════════════════════════════════════════════════════
     *
     * Η generateToken() παράγει ΝΕΟ token και αντικαθιστά το προηγούμενο στη
     * συνεδρία. Σωστό όταν καλείται μία φορά — καταστροφικό όταν μια σελίδα
     * έχει πολλές φόρμες.
     *
     * Η σελίδα προβολής αίτησης έχει ΤΕΣΣΕΡΙΣ φόρμες (προεπιλογή, πρόσληψη,
     * απόρριψη, απόσυρση). Κάθε μία καλούσε generateToken(). Η δεύτερη
     * κλήση ακύρωνε το token της πρώτης, η τρίτη της δεύτερης, και ούτω
     * καθεξής: μόνο το ΤΕΛΕΥΤΑΙΟ κουμπί της σελίδας δούλευε.
     *
     * Ο χρήστης που πατούσε «Πρόσληψη» — το πρώτο κουμπί — έπαιρνε
     * «Άκυρο αίτημα. Παρακαλώ δοκιμάστε ξανά.» Και δοκίμαζε ξανά, και
     * έπαιρνε το ίδιο, γιατί το πρόβλημα δεν ήταν στιγμιαίο: ήταν στη
     * σειρά των κλήσεων μέσα στη σελίδα.
     *
     * Η ασφάλεια δεν μειώνεται: το token παραμένει ανά συνεδρία, με λήξη,
     * και ελέγχεται με hash_equals. Απλώς δεν αυτοκαταστρέφεται.
     */
    public static function token(): string
    {
        ...
        if (Session::has('csrf_token')) {
            $age = time() - (int) Session::get('csrf_token_time', 0);
            if ($age <= 7200) {
