...
class JobTagModel {
    ...
    /**
 * Ανακτά τις ετικέτες (tags) μιας συγκεκριμένης αγγελίας
 * 
 * @param int $jobId Το ID της αγγελίας
 * @return array Πίνακας με τις ετικέτες
 */
/**
 * Ανακτά τις ετικέτες που σχετίζονται με μια συγκεκριμένη αγγελία
 * 
 * @param int $jobId Το ID της αγγελίας
 * @return array Πίνακας με τις ετικέτες
 */
public function getTagsByJobId($jobId) {
    try {
        // Χρήση των σωστών ονομάτων πινάκων και στηλών
        $sql = "SELECT t.* 
                FROM job_tags t
                JOIN job_listing_tags jlt ON t.id = jlt.job_tag_id
                WHERE jlt.job_listing_id = :job_id";
        ...
        error_log('Error getting tags for job listing: ' . $e->getMessage());
        return [];
    }
}
}