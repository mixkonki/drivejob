...
use PDO;
use PDOException;
use Drivejob\Core\Debug;  // Προσθέστε αυτή τη γραμμή
...
public function update($id, $data) {
    ...
    try {
        ...
        foreach ($data as $key => $value) {
            if ($key !== 'id') {  // Εξαιρούμε το id από τα πεδία που θα ενημερωθούν
                $updateParts[] = "$key = :$key";
                $params[$key] = $value;
...
public function getById($id) {
    try {
        ...
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute(['id' => $id]);
        return $stmt->fetch(PDO::FETCH_ASSOC); // Προσθήκη της ανάστροφης καθέτου
    } catch (\PDOException $e) { // Προσθήκη της ανάστροφης καθέτου
...
public function updateVehicleTypes($jobListingId, $vehicleTypes) {
    ...
    try {
        ...
        if (!is_array($vehicleTypes)) {
            ...
            $vehicleTypes = explode(',', $vehicleTypes); // Μετατροπή σε πίνακα αν είναι string
        }
